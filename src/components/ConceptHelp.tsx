...
const explanations: Record<string, string> = {
  한계생산물: '노동자 1명을 더 고용했을 때 추가로 늘어나는 생산량입니다. 노동자가 계속 늘면 보통 한계생산물은 감소합니다. 같은 기계에 새로운 노동자가 계속 들어오면 아무래도 생산량을 더 늘리기 어려워지겠죠.',
  한계비용: '제품을 1단위 더 생산할 때 추가로 드는 비용입니다. 노동자 1명의 임금은 그대로인데 노동자를 추가할수록 한계생산물은 감소하므로, 제품 1개를 추가로 생산하는 비용은 증가합니다. 경제학의 비용에는 기업을 계속 운영하는 데 필요한 최소 보상인 정상이윤도 포함해 생각합니다. 경쟁시장에서는 가격·평균수입·한계수입이 같으므로(P=AR=MR), 보통 MR=MC가 되는 생산량에서 이윤이 가장 커집니다.',

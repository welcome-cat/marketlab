...
import type { Company, InventoryItem, LearningReflection, ProductionPlan, Room, UpgradeType } from '../types/domain';
...
export const StudentPage: React.FC = () => {
  ...
  const quote = calculateProductionQuote(company, selectedMarket, plannedProductionQty, workerCount, machinePurchases, upgradePurchase, !inventory, room.currentRound, machineSales);
  ...
  const traitSelectionOpen = room.currentRound === 1 && room.roundPhase === 'DECISION' && room.status !== 'FINISHED';
  const canConfirm = isDecision && !plan && !submitting && (room.currentRound > 1 || company.traitsConfirmed !== false) && plannedProductionQty > 0 && workerCount >= minimumWorkerCount && quote.currentMarginalProduct > 0 && plannedProductionQty <= quote.productionCapacity && remainingBudget >= 0 && quote.machineCountAfter <= selectedMarket.maxMachines;
  ...
  const confirmProduction = async () => {
    ...
    try {
      ...
      await productionService.confirmProduction({ roomId, companyId: company.id, marketId: selectedMarket.id, requestedQuantity: plannedProductionQty, workerCount, machinePurchases, machineSales, upgradePurchase, askingPrice });
  ...
  const confirmTraits = async () => {
    ...
    } catch { setMessage('기업 특성을 확정하지 못했습니다. 1라운드 선택 시간이 끝났는지 확인해주세요.'); }
    finally { setSubmitting(false); }
  };
  ...
  return <div style={{ minHeight: '100vh', padding: '28px 16px', background: '#f8fafc' }}><main className="student-dashboard">
    ...
    <section className="student-round" style={{ ...card, border: `1px solid ${isRunning ? '#86efac' : '#fde68a'}`, background: isRunning ? '#f0fdf4' : '#fffbeb' }}>
      ...
      <span style={{ display: 'block', fontSize: '13px', color: '#64748b', marginTop: '4px' }}>1라운드는 4개월입니다. 카페·운동화·스마트폰은 매 라운드, 쌀 생산량은 3라운드마다 조절합니다.</span>
    </section>
    ...
    {room.currentRound === 1 && <section className="student-diagnosis" style={{ ...card, border: '2px solid #0f766e', background: '#f0fdfa', gridColumn: '1 / -1' }}>
      <h2 style={{ marginTop: 0, fontSize: '18px' }}>🔎 생산 시작 전 우리 기업 진단서</h2>
      {!company.traitsConfirmed ? <div>
        ...
        <p style={{ color: '#475569', fontSize: '13px' }}>{company.industryTraitIcon} <strong>{company.industryTraitName}</strong>. 노동자와 희망 생산량을 움직여 실제 돈을 쓰지 않고 시장별 결과를 비교하세요.</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: '10px' }}>{room.markets.map((market) => {
          const capacityQuote = calculateProductionQuote(company, market, 1, diagnosisWorkers);
          const simulatedQuantity = Math.min(capacityQuote.productionCapacity, diagnosisQuantities[market.id] || Math.max(1, Math.round(capacityQuote.productionCapacity * 0.6)));
          const sample = calculateProductionQuote(company, market, simulatedQuantity, diagnosisWorkers);
          const fit = getTechnologyMarketFit(company, market);
          const expectedProfit = market.announcedPrice * simulatedQuantity - sample.economicCost;
          const unit = market.id === 'market_toy' ? '포대' : '개';
          return <article key={market.id} style={{ padding: '13px', background: '#fff', border: '1px solid #99f6e4', borderRadius: '10px' }}><strong>{market.icon} {market.name}</strong><label style={{ display: 'block', marginTop: '8px', fontSize: '12px' }}>모의 생산량 {simulatedQuantity.toLocaleString()}{unit}<input type="range" min="1" max={Math.max(1, capacityQuote.productionCapacity)} value={simulatedQuantity} onChange={(event) => setDiagnosisQuantities((current) => ({ ...current, [market.id]: Number(event.target.value) }))} style={{ width: '100%' }} /></label><span style={{ display: 'block' }}>생산 가능량 <b style={{ float: 'right' }}>{capacityQuote.productionCapacity.toLocaleString()}{unit}</b></span><span style={{ display: 'block' }}>평균비용 <b style={{ float: 'right' }}>{sample.averageCost.toLocaleString()}원</b></span><span style={{ display: 'block' }}>한계비용 <b style={{ float: 'right' }}>{sample.marginalCost?.toLocaleString() || '-'}원</b></span><span style={{ display: 'block' }}>전량 판매 시 이윤 <b style={{ float: 'right', color: expectedProfit >= 0 ? '#059669' : '#dc2626' }}>{expectedProfit.toLocaleString()}원</b></span><small style={{ display: 'block', marginTop: '8px', color: '#0f766e' }}>{fit.hint}</small></article>;
        })}</div>
        <p style={{ marginBottom: 0, fontSize: '12px', color: '#64748b' }}>모의 이윤은 비교를 위한 값이며 실제 수요·경쟁·미판매 결과에 따라 달라질 수 있습니다.</p>
      </div>}
    </section>}
    ...
    <section className="student-investment" style={card}><h2 style={{ marginTop: 0, fontSize: '18px' }}>2. 설비와 기술에 투자할 것인가?</h2>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: '12px' }}>
        <label style={{ padding: '14px', background: '#f8fafc', borderRadius: '10px' }}>새 {selectedMarket.id === 'market_toy' ? '농기계' : '기계'} 구입 대수
          <input type="number" min="0" step="1" value={room.currentRound < room.unlockRounds.machines ? 0 : machinePurchases} disabled={Boolean(plan) || room.currentRound < room.unlockRounds.machines} onChange={(e) => setMachinePurchases(Math.max(0, Math.floor(Number(e.target.value) || 0)))} style={{ width: '100%', marginTop: '8px', padding: '9px' }} />

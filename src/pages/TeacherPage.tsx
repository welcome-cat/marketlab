...
import { calculateCompetitiveMarket, companyService, newsService, productionService, reflectionService, roomService, scaleMarketEventFactor } from '../services';
...
export const TeacherPage: React.FC = () => {
  ...
  const eventForecasts = activeRoom?.markets.map((market) => {
    ...
    const setting = marketInfluenceDraft[market.id] || market;
    const demandMultiplier = option.effectType === 'SUPPLY' ? 1 : scaleMarketEventFactor(option.multiplier, setting.demandEventEffectScale);
    const supplyMultiplier = scaleMarketEventFactor(supplyOption.supplyMultiplier, setting.supplyEventEffectScale);
    ...
    return {
      ...
      materialCost: supplyOption.id.startsWith('material_') && materialPriceDraft[market.id] ? materialPriceDraft[market.id] : Math.round(market.materialUnitCost * market.materialCostMultiplier * scaleMarketEventFactor(supplyOption.materialMultiplier, setting.supplyEventEffectScale)),
      wage: supplyOption.id.startsWith('wage_') && wageDraft[market.id] ? wageDraft[market.id] : Math.round(market.wagePerWorker * scaleMarketEventFactor(supplyOption.wageMultiplier, setting.supplyEventEffectScale)),
      productivity: Math.round(market.firstWorkerProductivity * scaleMarketEventFactor(supplyOption.productivityMultiplier, setting.supplyEventEffectScale) * 100) / 100,
    };
  }) || [];
      ...
      {!activeRoom ? <div style={{ display: 'grid', gap: '20px' }}>
        ...
        <section style={{ ...card, border: '2px solid #7c3aed', background: '#faf5ff' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: '12px' }}>
            {activeRoom.markets.map((market) => {
              ...
              const demandMultiplier = activeEvent?.effectType === 'SUPPLY' ? 1 : scaleMarketEventFactor(activeEvent?.multiplier, setting.demandEventEffectScale);
              const supplyMultiplier = activeEvent?.effectType === 'SUPPLY' ? scaleMarketEventFactor(activeEvent?.supplyMultiplier, setting.supplyEventEffectScale) : 1;
        ...
        <section className="teacher-demand-events" style={{ ...card, border: '2px solid #d97706', background: '#fffbeb' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', alignItems: 'start', flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}><button disabled={newsGenerating || !canPrepareDemandEvents} onClick={handleRandomDemandEvents}>🎲 수요·공급 사건 무작위 선택</button><button onClick={() => setShowForecast((current) => !current)}>🔍 {showForecast ? '예측 닫기' : '예측 확인'}</button><button disabled={newsGenerating || !canPrepareDemandEvents} onClick={handleConfirmDemandEvents} style={{ background: '#d97706', color: '#fff', border: 0, borderRadius: '8px', padding: '10px 14px', fontWeight: 800 }}>{newsGenerating ? '기사 작성 중...' : '선택 확정·신문 발행'}</button></div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(230px,1fr))', gap: '12px', marginTop: '14px' }}>{activeRoom.markets.map((market) => {
            ...
            return <article key={market.id} style={{ background: '#fff', padding: '13px', borderRadius: '10px', border: '1px solid #fcd34d' }}><strong>{market.icon} {market.name}</strong><label style={{ display: 'block', marginTop: '8px', fontSize: '12px' }}>수요변동<select value={demandSelections[market.id] || 'baseline'} onChange={(event) => setDemandSelections((current) => ({ ...current, [market.id]: event.target.value }))} style={{ width: '100%', marginTop: '4px', padding: '9px' }}>{DEMAND_EVENT_OPTIONS.filter((option) => option.effectType !== 'SUPPLY').map((option) => <option key={option.id} value={option.id}>{option.title}</option>)}</select></label><label style={{ display: 'block', marginTop: '8px', fontSize: '12px' }}>공급변동<select value={supplySelections[market.id] || 'supply_baseline'} onChange={(event) => setSupplySelections((current) => ({ ...current, [market.id]: event.target.value }))} style={{ width: '100%', marginTop: '4px', padding: '9px' }}>{supplyOptions.map((option) => <option key={option.id} value={option.id}>{option.title}</option>)}</select></label>{selectedOption.factor !== 'BASELINE' && <><span style={{ display: 'block', marginTop: '9px', fontSize: '12px' }}>수요변동 크기: <b>{Math.round(setting.demandEventEffectScale * 100)}%</b></span><input aria-label={`${market.name} 수요변동 크기`} type="range" min="0" max="1000" step="10" value={Math.round(setting.demandEventEffectScale * 100)} onChange={(event) => setMarketInfluenceDraft((current) => ({ ...current, [market.id]: { ...setting, demandEventEffectScale: Number(event.target.value) / 100 } }))} /></>}{selectedSupplyOption.factor !== 'BASELINE' && <><span style={{ display: 'block', marginTop: '9px', fontSize: '12px' }}>공급변동 크기: <b>{Math.round(setting.supplyEventEffectScale * 100)}%</b></span><input aria-label={`${market.name} 공급변동 크기`} type="range" min="0" max="1000" step="10" value={Math.round(setting.supplyEventEffectScale * 100)} onChange={(event) => setMarketInfluenceDraft((current) => ({ ...current, [market.id]: { ...setting, supplyEventEffectScale: Number(event.target.value) / 100 } }))} /></>}{selectedSupplyOption.id.startsWith('material_') && <label style={{ display: 'block', marginTop: '8px' }}>변경 후 단위 재료비<input type="number" min="0" value={materialPriceDraft[market.id] || Math.round(market.materialUnitCost * market.materialCostMultiplier)} onChange={(event) => setMaterialPriceDraft((current) => ({ ...current, [market.id]: Number(event.target.value) }))} style={{ width: '100%' }} /></label>}{selectedSupplyOption.id.startsWith('wage_') && <label style={{ display: 'block', marginTop: '8px' }}>변경 후 라운드 임금<input type="number" min="0" value={wageDraft[market.id] || market.wagePerWorker} onChange={(event) => setWageDraft((current) => ({ ...current, [market.id]: Number(event.target.value) }))} style={{ width: '100%' }} /></label>}{selectedSupplyOption.id.startsWith('rent_') && <label style={{ display: 'block', marginTop: '8px' }}>변경 후 임대료<input type="number" min="0" value={rentDraft[market.id] || market.rentPerRound} onChange={(event) => setRentDraft((current) => ({ ...current, [market.id]: Number(event.target.value) }))} style={{ width: '100%' }} /></label>}{selectedSupplyOption.id === 'producer_tax' && <label style={{ display: 'block', marginTop: '8px' }}>단위당 세금<input type="number" min="0" value={taxDraft[market.id] || 0} onChange={(event) => setTaxDraft((current) => ({ ...current, [market.id]: Number(event.target.value) }))} style={{ width: '100%' }} /></label>}{selectedSupplyOption.id === 'producer_subsidy' && <label style={{ display: 'block', marginTop: '8px' }}>단위당 보조금<input type="number" min="0" value={subsidyDraft[market.id] || 0} onChange={(event) => setSubsidyDraft((current) => ({ ...current, [market.id]: Number(event.target.value) }))} style={{ width: '100%' }} /></label>}{selectedSupplyOption.id === 'rice_typhoon' && <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px', marginTop: '8px' }}><label>피해 확률(%)<input type="number" min="0" max="100" value={disasterChanceDraft[market.id] || 40} onChange={(event) => setDisasterChanceDraft((current) => ({ ...current, [market.id]: Number(event.target.value) }))} style={{ width: '100%' }} /></label><label>피해율(%)<input type="number" min="0" max="100" value={disasterLossDraft[market.id] || 30} onChange={(event) => setDisasterLossDraft((current) => ({ ...current, [market.id]: Number(event.target.value) }))} style={{ width: '100%' }} /></label></div>}</article>;
          })}</div>
        ...
        <section className="teacher-curves" style={card}><h3 style={{ marginTop: 0 }}>📉 가격수용 시장 수요·공급곡선</h3><p style={{ color: '#64748b', fontSize: '13px' }}>스마트폰 과점시장에는 하나의 공급곡선을 적용하지 않습니다.</p><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(320px,1fr))', gap: '18px' }}>{activeRoom.markets.filter((market) => market.marketType === 'PERFECT_COMPETITION').map((market) => { const event = activeRoom.demandEvents.find((item) => item.marketId === market.id); const demandMultiplier = event?.effectType === 'SUPPLY' ? 1 : scaleMarketEventFactor(event?.multiplier, market.demandEventEffectScale); return <MarketCurveChart key={market.id} market={market} plans={roundPlans.filter((plan) => plan.productId === market.id)} demandMultiplier={demandMultiplier} />; })}</div></section>
        ...
        {roundPlans.some((item) => item.settlementStatus === 'SETTLED') && <section style={card}><h3 style={{ marginTop: 0 }}>🏆 Round {activeRoom.currentRound} 기업별 이윤</h3><div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'center', fontSize: '13px' }}><thead><tr><th>순위</th><th>기업</th><th>시장</th><th>판매량</th><th>매출</th><th>이윤</th></tr></thead><tbody>{roundPlans.filter((item) => item.settlementStatus === 'SETTLED').sort((a, b) => (b.economicProfit ?? b.profit ?? 0) - (a.economicProfit ?? a.profit ?? 0)).map((item, index) => <tr key={item.id} style={{ borderTop: '1px solid #e2e8f0' }}><td style={{ padding: '9px' }}>{index + 1}</td><td>{companies.find((company) => company.id === item.companyId)?.name || item.companyId}</td><td>{item.marketName}</td><td>{(item.soldQuantity || 0).toLocaleString()}개</td><td>{(item.revenue || 0).toLocaleString()}원</td><td style={{ color: (item.economicProfit ?? item.profit ?? 0) >= 0 ? '#059669' : '#dc2626', fontWeight: 800 }}>{(item.economicProfit ?? item.profit ?? 0).toLocaleString()}원</td></tr>)}</tbody></table></div></section>}
